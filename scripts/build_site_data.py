...
def snapshot(rows):
    ...
    return {
        ...
        "ppg": points / n,
        "fh": sum(r["fh"] for r in rows) / n,
        "sh": sum(r["sh"] for r in rows) / n,
        "full": sum(r["full"] for r in rows) / n,
        "form": form,
    }


def over_25_probability(home, away):
    """Estimate P(total goals >= 3) using a Poisson total-goals model."""
    home_xg = max(0.2, (home["gf"] + away["ga"]) / 2)
    away_xg = max(0.2, (away["gf"] + home["ga"]) / 2)
    total_lambda = min(5.0, max(0.8, home_xg + away_xg))
    under_or_equal_2 = math.exp(-total_lambda) * (
        1 + total_lambda + (total_lambda ** 2) / 2
    )
    return max(0.05, min(0.95, 1 - under_or_equal_2))
...
def probs(home, away, league_fh, league_sh):
    ...
    over25 = over_25_probability(home, away)
    confidence = max(.50, min(.86, .50 + .024 * min(home["n"], away["n"], 10) + .12 * max(abs(fh - .5), abs(sh - .5), abs(over25 - .5))))
    return p, fh, sh, over25, confidence


def main():
    if not os.getenv("SPORTMONKS_API_TOKEN"):
        print("No API token; existing site data kept.")
        return
    ...
    for f in fixtures:
        ...
        if hf is None or af is None or hh is None or ah is None or f.get("starting_at", "")[:10] >= today.isoformat():
            continue
        fh = int(hh > 0 and ah > 0)
        sh = int((hf - hh) > 0 and (af - ah) > 0)
        full = int(hf > 0 and af > 0)
    ...
    n = len(completed)
    lfh = sum(x["fh"] for x in completed) / n if n else .25
    lsh = sum(x["sh"] for x in completed) / n if n else .35

    upcoming = []
    for f in fixtures:
        if f.get("starting_at", "")[:10] < today.isoformat():
            continue
        h, a = participant(f, "home"), participant(f, "away")
        if not h or not a:
            continue
        hp, ap = snaps.get(h["id"], snapshot([])), snaps.get(a["id"], snapshot([]))
        p, fh, sh, over25, conf = probs(hp, ap, lfh, lsh)
        upcoming.append({
            "match_id": f.get("id"),
            "home_team": h.get("name"),
            "away_team": a.get("name"),
            "kickoff": kickoff_iso(f.get("starting_at")),
            "home_win": p[0],
            "draw": p[1],
            "away_win": p[2],
            "over_2_5": over25,
            ...
            "second_half_btts": sh,
            "confidence": conf,
            "model_label": "Trend + Poisson",
        })
        if len(upcoming) == 10:
            break

    teams = [{
        "team": names.get(k, str(k)),
        "matches": v["n"],
        "goals_for_avg": v["gf"],
        "goals_against_avg": v["ga"],
        "first_half_btts_rate": v["fh"],
        "second_half_btts_rate": v["sh"],
        "full_match_btts_rate": v["full"],
        "form": v["form"],
    } for k, v in sorted(snaps.items(), key=lambda x: names.get(x[0], ""))]

    payload = {
        "mode": "live",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "model": "trend-poisson-v1",
        "league": {
            "matches": n,
            "goals_per_match": sum(x["gf"] + x["ga"] for x in completed) / n if n else 0,
            "first_half_btts_rate": lfh,
            "second_half_btts_rate": lsh,
        },
        "matches": upcoming,
        "teams": teams,
    }
